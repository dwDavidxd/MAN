...
parser = argparse.ArgumentParser(description='PyTorch Adaptive Test')
...
def craft_adversarial_example_pgd_plus(model, x_natural, y, step_size=0.003,
                epsilon=0.031, perturb_steps=10, distance='l_inf'):
    ...
    if distance == 'l_inf':
        for _ in range(perturb_steps):
            ...
            with torch.enable_grad():
                ...
                loss = F.cross_entropy(ori_logits, y) + \
                           nn.NLLLoss()(log_noisy_post, y)
...
def craft_adversarial_example(model, x_natural, y, step_size=0.003,
    ...
    adversary = CarliniWagnerL2Attack(
        model, 10, clip_min=0.0, clip_max=1.0, max_iterations=40, confidence=0, initial_const=1, learning_rate=1e-2,
...
def evaluate_at(i, data, label, model):
    ...
    data_adv = craft_adversarial_example(model=model, x_natural=data, y=label, step_size=args.step_size,
                                         epsilon=args.epsilon, perturb_steps=40)
    ...
    logits, logits_ori = model(data_adv, out_all=True)
    outputs = F.softmax(logits, dim=1)

    _, pred = torch.max(outputs.data, 1)
    _, pred_adv_ori = torch.max(logits_ori.data, 1)

    correct_adv = (pred == label).sum()
    correct_adv_ori = (pred_adv_ori == label).sum()

    acc1 = 100 * float(correct1) / data.size(0)

    acc_adv = 100 * float(correct_adv) / data.size(0)
    acc_adv_ori = 100 * float(correct_adv_ori) / data.size(0)

    if (i + 1) % args.print_freq == 0:
        print(
            'Iter [%d/%d] Test classifier: Nat Acc: %.4f; Adv Acc: %.4f; Adv Acc ori: %.4f'
            % (i + 1, 10000 // data.size(0), acc1, acc_adv, acc_adv_ori))

    return [acc1, acc_adv, acc_adv_ori]
...
def main(args):
    ...
    classifier = ResNet18(10, 100).to(device)
    classifier.load_state_dict(torch.load(os.path.join(model_dir, 'best_model.pth')))
    ...
    for i, (data, labels) in enumerate(test_loader):
        ...
        test_adv_correct_ori += test_acc[2]

    test_nat_correct = test_nat_correct / total_num
    test_adv_correct = test_adv_correct / total_num
    test_adv_correct_ori = test_adv_correct_ori / total_num

    print('Test Classifer: Nat ACC: %.4f; Adv ACC: %.4f; Adv ori ACC: %.4f' % (test_nat_correct, test_adv_correct,
                                                                           test_adv_correct_ori))
