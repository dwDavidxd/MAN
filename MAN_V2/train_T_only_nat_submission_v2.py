...
parser = argparse.ArgumentParser(description='PyTorch CIFAR Standard_MAN Adversarial Training')
...
def craft_adversarial_example_overall(classifier, T_model, x_natural, y, step_size=2 / 255, epsilon=8 / 255, perturb_steps=10):
...
def standard_loss(classifier, T_model, x_natural, y, optimizer, step_size=2/255, epsilon=8/255, perturb_steps=10, distance='l_inf'):
    ...
    x_adv_all = craft_adversarial_example_overall(classifier, T_model, x_natural, y, step_size=step_size, epsilon=epsilon,
                                                   perturb_steps=perturb_steps)
    ...
    # adversarial loss for x_adv
    logits = classifier(x_adv)#.detach()
    T_adv = T_model(x_adv)

    pred_labels = F.softmax(logits, dim=1)
    noisy_post = torch.bmm(pred_labels.unsqueeze(1), T_adv).squeeze(1)  # softmax output

    logits_adv = torch.log(noisy_post + 1e-12)
    loss_adv = nn.NLLLoss()(logits_adv, y)

    # T_loss
    T_adv_all = T_model(x_adv_all)
    T_row = torch.nn.functional.one_hot(y, 10).float()
    T_row = T_row.unsqueeze(1)
    T_label = T_row.repeat(1, 10, 1)

    loss_T = nn.MSELoss()(T_adv_all, T_label)

    loss = loss_adv + 800.0 * loss_T
...
def eval_test(classifier, T_model, device, test_loader):
    classifier.eval()
    T_model.eval()
    ...
    for data, target in test_loader:
        data, target = data.to(device), target.to(device)
        data_adv = craft_adversarial_example_overall(classifier=classifier, T_model=T_model, x_natural=data, y=target,
                                                     step_size=2/255, epsilon=8/255, perturb_steps=20)

        logits = classifier(data)
        T = T_model(data)
        pred_labels = F.softmax(logits, dim=1)
        output = torch.bmm(pred_labels.unsqueeze(1), T).squeeze(1)  # softmax output

        logits_adv = classifier(data_adv)
        T_adv = T_model(data_adv)
        pred_labels_adv = F.softmax(logits_adv, dim=1)
        output_adv = torch.bmm(pred_labels_adv.unsqueeze(1), T_adv).squeeze(1)  # softmax output
        output_adv_ori = logits_adv.clone()
    ...
    print('Test-adaptive: Accuracy: {}/{} ({:.0f}%), Robust Accuracy: {}/{} ({:.0f}%), Robust ori Accuracy:'
    ...
    for data, target in test_loader:
        data, target = data.to(device), target.to(device)
        data_adv = craft_adversarial_example(model=classifier, x_natural=data, y=target,
                                                     step_size=2/255, epsilon=8/255, perturb_steps=20)
    ...
    print('Test-general: Robust Accuracy: {}/{} ({:.0f}%), Robust ori Accuracy:'
          ' {}/{} ({:.0f}%)'.format(correct_adv, len(test_loader.dataset),
        100. * correct_adv / len(test_loader.dataset), correct_adv_ori, len(test_loader.dataset),
        100. * correct_adv_ori / len(test_loader.dataset)))
...
def main():
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if epoch % args.save_freq == 0:
            print('================================================================')
            eval_test(classifier, T_model, device, test_loader)
